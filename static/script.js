...
// 4択モード用変数
let isFourChoiceMode = false;
let currentQuestionStartTime = 0;
let fourChoiceTimer = null;
let fourChoiceDelayTimer = null;
let fourChoiceTimeLimitMs = 7000;
...
const correctWordsContainer = document.getElementById('correctWordsContainer');
const correctWordsToggle = document.getElementById('correctWordsToggle');
const correctWordsContent = document.getElementById('correctWordsContent');
const correctWordsChevron = document.getElementById('correctWordsChevron');
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                setTimeout(() => {
                    ...
                    updateUnitCheckboxStates(); // 履歴ロード後にZ問題の解放状態を再評価する
                }, 500);
            } else {
...
function loadWordDataFromServer() {
    fetch('/api/global_question_stats')
        .then(response => response.json())
        .then(data => {
...
function checkSpecialUnlockClientSide(chapterNum) {
    // 同じ章の通常問題（Z以外）で、有効かつ答えが空でないものを取得
    const regularProblems = word_data.filter(word => {
        if (String(word.chapter) !== String(chapterNum)) return false;
        if (String(word.number).toUpperCase() === 'Z') return false;
        if (word.enabled === false) return false;
        if (!word.answer || String(word.answer).trim() === '') return false;
        return true;
    });
    ...
    for (const word of regularProblems) {
        ...
        const total = history.attempts !== undefined ? history.attempts : (correct + incorrect);
...
function getSelectedQuestions() {
    ...
    document.querySelectorAll('.unit-item input[type="checkbox"]:checked').forEach(checkbox => {
        selectedUnits.add(`${String(checkbox.dataset.chapter).trim()}-${String(checkbox.value).trim()}`);
    });

    return word_data.filter(word => {
        const unitIdentifier = `${String(word.chapter).trim()}-${String(word.number).trim()}`;
...
function getFilteredQuestions() {
    ...
    if (isIncorrectOnly) {
        ...
        // ★ 完璧チェックによる除外処理（通常モードのみ）
        quizQuestions = quizQuestions.filter(word => {
            const wordIdentifier = generateProblemId(word);
            const history = problemHistory[wordIdentifier];
            if (history && history.perfect_until) {
                if (history.perfect_until === 'forever') {
                    return false;
                }
                const untilDate = new Date(history.perfect_until);
...
function updateSelectionTotalCount() {
    ...
    if (count > 0) {
        ...
        // 0問になった原因を特定してヒントを表示
        const selectedRaw = getSelectedQuestions();
        if (selectedRaw.length > 0) {
            countSpan.textContent = '(0問 - 完璧チェック済み等のため)';
...
function startQuiz() {
    try {
        ...
        // 苦手問題モードでもカウント制限を行う
        const orderOrdered = document.getElementById('orderOrdered')?.checked || false;
        const orderCustom = document.getElementById('orderCustom')?.checked || false;
        ...
        if (orderOrdered) {
            ...
            currentQuizData = [...quizQuestions].sort((a, b) => {
                const idA = generateProblemId(a);
                const idB = generateProblemId(b);
                const valA = customOrder[idA] !== undefined ? customOrder[idA] : 999999;
                const valB = customOrder[idB] !== undefined ? customOrder[idB] : 999999;
                return valA - valB;
            });
        } else {
...
function showNextQuestion() {
    if (fourChoiceTimer) {
        clearTimeout(fourChoiceTimer);
        fourChoiceTimer = null;
    }
    ...
    const perfectContainer = document.getElementById('perfectCheckContainer');
    const isIncorrectOnlyMode = document.getElementById('incorrectOnlyCheckbox2')?.checked || false;
    if (perfectContainer) {
        perfectContainer.classList.add('hidden');

        const checkbox = document.getElementById('perfectCheckbox');
        const select = document.getElementById('perfectDurationSelect');
        if (checkbox) checkbox.checked = false;
        if (select) {
            const savedDuration = localStorage.getItem('perfectDuration') || '1month';
            select.value = savedDuration;
        }
    }
    ...
    if (currentQuestionIndex < totalQuestions) {
        ...
        if (questionElement) {
            ...
            // 全体正答率の表示更新
            const globalAccDisplay = document.getElementById('globalAccuracyDisplay');
            const globalAccValue = document.getElementById('globalAccuracyValue');
            if (globalAccDisplay && globalAccValue) {
                const globalRate = globalProblemStats[wordIdentifier];
                if (globalRate !== undefined) {
                    globalAccValue.textContent = globalRate;
                    globalAccDisplay.style.display = 'block';
                } else {
                    globalAccDisplay.style.display = 'none';
                }
            }
            ...
            // --- マスター判定 & 4択モード切り替え ---
            const history = problemHistory[wordIdentifier] || { correct_attempts: 0, incorrect_attempts: 0 };
            const totalAttempts = history.attempts !== undefined ? (history.attempts || 0) : ((history.correct_attempts || 0) + (history.incorrect_attempts || 0));
            const isMastered = totalAttempts > 0 && (history.correct_attempts / totalAttempts) >= 0.5;

            isFourChoiceMode = !isMastered && (currentWord.dummy_options && currentWord.dummy_options.length > 0);

            const selfEvalContainer = document.getElementById('selfEvaluationContainer');
            const fourChoiceContainer = document.getElementById('fourChoiceContainer');
            const timerBarContainer = document.getElementById('timerBarContainer');
            const timerBar = document.getElementById('timerBar');

            if (isFourChoiceMode) {
                if (selfEvalContainer) selfEvalContainer.classList.add('hidden');
                if (fourChoiceContainer) fourChoiceContainer.classList.remove('hidden');
                if (timerBarContainer) timerBarContainer.classList.remove('hidden');

                // 4択モード中はマイク・手書きボタンを非表示にしてレイアウトを保護
                const voiceBtn = document.getElementById('voiceAnswerBtn');
                const voiceBtnMobile = document.getElementById('voiceAnswerBtnMobile');
                const openMemoBtn = document.getElementById('openMemoBtn');
                if (voiceBtn) voiceBtn.classList.add('hidden');
                if (voiceBtnMobile) voiceBtnMobile.classList.add('hidden');
                if (openMemoBtn) openMemoBtn.classList.add('hidden');

                // 選択肢のシャッフル
                let options = [currentWord.answer];
                let dummies = [...currentWord.dummy_options];
                dummies = shuffleArray(dummies).slice(0, 3);
                options = options.concat(dummies);
                ...
                for (let i = 0; i < 4; i++) {
                    ...
                    if (btn) {
                        if (i < options.length) {
                            ...
                            btn.className = 'choice-btn'; // リセット (secondary-buttonを除去して文字色同化を回避)
                            btn.disabled = false;
                            ...
                            btn.onclick = () => {
                                // クリック後は全ボタンを無効化
                                for (let j = 0; j < 4; j++) {
                                    const b = document.getElementById(`choiceBtn${j}`);
                                ...
                                if (timerBar) {
                                    ...
                                    timerBar.style.backgroundColor = computedBg;
                                }

                                const isCorrect = (options[i] === currentWord.answer);
                                if (isCorrect) {
                                    ...
                                    for (let j = 0; j < 4; j++) {
                                        const b = document.getElementById(`choiceBtn${j}`);
                                        if (b && b.textContent === currentWord.answer) {
                                            b.classList.add('correct-reveal');
                                        }
                                    }
                                }

                                // 1.2秒間、正誤表示を見せてから回答処理へ
                                fourChoiceDelayTimer = setTimeout(() => {
                ...
                if (timerBar) {
                    ...
                    setTimeout(() => {
                        timerBar.style.transition = `width ${fourChoiceTimeLimitMs}ms linear, background-color ${fourChoiceTimeLimitMs}ms`;
                        timerBar.style.width = '0%';
                        timerBar.style.backgroundColor = '#e74c3c';
                    }, 50);
                }

                fourChoiceTimer = setTimeout(() => {
                    if (isFourChoiceMode) {
                        // 正解のボタンをハイライト
                        for (let j = 0; j < 4; j++) {
                            const b = document.getElementById(`choiceBtn${j}`);
                            if (b) {
                                b.disabled = true;
                                if (b.textContent === currentWord.answer) {
                                    b.classList.add('correct-reveal');
                                }
                            }
                        }
                        // タイムオーバーの場合も正答確認の時間（1.2秒）を与えてから処理
                        fourChoiceDelayTimer = setTimeout(() => {
                            handleAnswer(false);
                        }, 1200);
                    }
                }, fourChoiceTimeLimitMs);

            } else {
                if (selfEvalContainer) selfEvalContainer.classList.remove('hidden');
                if (fourChoiceContainer) fourChoiceContainer.classList.add('hidden');
                if (timerBarContainer) timerBarContainer.classList.add('hidden');

                // 通常モード時はマイク・手書きボタンの非表示を解除
                const voiceBtn = document.getElementById('voiceAnswerBtn');
                const voiceBtnMobile = document.getElementById('voiceAnswerBtnMobile');
                const openMemoBtn = document.getElementById('openMemoBtn');
                if (voiceBtn) voiceBtn.classList.remove('hidden');
                if (voiceBtnMobile) voiceBtnMobile.classList.remove('hidden');
                if (openMemoBtn) openMemoBtn.classList.remove('hidden');
        ...
        if (answerElement) answerElement.textContent = ''; // DOMからカンニングできないように空にしておく
    } else {
        showQuizResult();
    }
}
...
function showAnswer() {
    ...
    if (answerElement) {
        // ボタンが押されたこの瞬間に、初めてDOMに答えの文字をセットする
        answerElement.textContent = currentQuizData[currentQuestionIndex].answer;
    ...
    const isIncorrectOnlyMode = document.getElementById('incorrectOnlyCheckbox2')?.checked || false;
    if (perfectContainer) {
        if (isIncorrectOnlyMode || isFourChoiceMode) {
            perfectContainer.classList.add('hidden');
            perfectContainer.style.display = 'none';
        } else {
            perfectContainer.classList.remove('hidden');
            perfectContainer.style.display = 'flex';
        }
...
function handleAnswer(isCorrect) {
    ...
    } else if (problemHistory[wordIdentifier].attempts === undefined) {
        problemHistory[wordIdentifier].attempts = (problemHistory[wordIdentifier].correct_attempts || 0) + (problemHistory[wordIdentifier].incorrect_attempts || 0);
    }

    problemHistory[wordIdentifier].attempts++;
    problemHistory[wordIdentifier].last_answered = new Date().toISOString();
    currentSessionAnswered.add(wordIdentifier);

    // 完璧チェックの処理（苦手問題モードでない場合のみ）
    const isIncorrectOnlyModeForPerfect = document.getElementById('incorrectOnlyCheckbox2')?.checked || false;
    if (!isIncorrectOnlyModeForPerfect) {
        const perfectCheckbox = document.getElementById('perfectCheckbox');
        const durationSelect = document.getElementById('perfectDurationSelect');
        if (perfectCheckbox && perfectCheckbox.checked) {
            const duration = durationSelect ? durationSelect.value : '1month';
            if (durationSelect) localStorage.setItem('perfectDuration', duration);
            
            if (duration === 'forever') {
                problemHistory[wordIdentifier].perfect_until = 'forever';
            } else {
                const date = new Date();
                if (duration === '1month') date.setMonth(date.getMonth() + 1);
                else if (duration === '2weeks') date.setDate(date.getDate() + 14);
                else if (duration === '1week') date.setDate(date.getDate() + 7);
                else if (duration === '3days') date.setDate(date.getDate() + 3);
                problemHistory[wordIdentifier].perfect_until = date.toISOString();
            }
        } else if (problemHistory[wordIdentifier].perfect_until) {
            // チェックを外した場合（再回答時などに上書きされる場合）
            delete problemHistory[wordIdentifier].perfect_until;
        }
    }

    // タイムボーナスの計算 (4択モード正解時のみ)
    let bonusMultiplier = 1.0;
    if (isFourChoiceMode && isCorrect) {
    ...
    if (isCorrect) {
        correctCount++;
        problemHistory[wordIdentifier].correct_attempts += bonusMultiplier;
...
function triggerStrategicRetreat() {
    ...
    // 4択モードのタイマーを確実に停止
    if (typeof fourChoiceTimer !== 'undefined' && fourChoiceTimer) {
        clearTimeout(fourChoiceTimer);
        fourChoiceTimer = null;
    }
    if (typeof fourChoiceDelayTimer !== 'undefined' && fourChoiceDelayTimer) {
        clearTimeout(fourChoiceDelayTimer);
        fourChoiceDelayTimer = null;
    }
...
function showQuizResult() {
    ...
    displayCorrectWordsForCurrentQuiz();
...
// 正解問題表示関数
function displayCorrectWordsForCurrentQuiz() {
    if (!correctWordsContainer) return;

    correctWordsContainer.innerHTML = '';
    const currentQuizCorrectWords = [];

    if (correctCount === 0) {
        const correctWordListElement = document.getElementById('correctWordList');
        if (correctWordListElement) correctWordListElement.classList.add('hidden');
        return;
    }

    currentQuizData.forEach(word => {
        const wordIdentifier = generateProblemId(word);
        if (currentSessionAnswered.has(wordIdentifier) && problemHistory[wordIdentifier] && problemHistory[wordIdentifier].correct_streak > 0) {
            currentQuizCorrectWords.push(word);
        }
    });

    const correctWordListElement = document.getElementById('correctWordList');
    if (currentQuizCorrectWords.length > 0) {
        if (correctWordListElement) correctWordListElement.classList.remove('hidden');
        currentQuizCorrectWords.forEach((word, index) => {
            const li = document.createElement('li');
            li.innerHTML = `
                <div class="incorrect-question">${word.question}</div>
                <div class="incorrect-answer-container">
                    <span class="incorrect-answer hidden" id="correct-answer-${index}" style="color: #2ecc71;">${word.answer}</span>
                    <button class="show-incorrect-answer-button" onclick="toggleCorrectAnswer(${index})" style="background-color: #2ecc71;">答えを見る</button>
                </div>
            `;
            correctWordsContainer.appendChild(li);
        });
    } else {
        if (correctWordListElement) correctWordListElement.classList.add('hidden');
    }
...
function toggleCorrectAnswer(index) {
    const answerElement = document.getElementById(`correct-answer-${index}`);
    const button = answerElement ? answerElement.nextElementSibling : null;
...
// イベントリスナー: 正解リストの折りたたみ
if (correctWordsToggle) {
    correctWordsToggle.addEventListener('click', () => {
        if (correctWordsContent.style.display === 'none') {
            correctWordsContent.style.display = 'block';
            correctWordsChevron.classList.replace('fa-chevron-down', 'fa-chevron-up');
        } else {
            correctWordsContent.style.display = 'none';
            correctWordsChevron.classList.replace('fa-chevron-up', 'fa-chevron-down');
        }
    });
}
...
document.addEventListener('keydown', (event) => {
    if (typeof isFourChoiceMode !== 'undefined' && isFourChoiceMode) return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const orderOrdered = document.getElementById('orderOrdered');
    const savedOrder = localStorage.getItem('questionOrder') || 'random';
    
    if (savedOrder === 'ordered' && orderOrdered) {
        orderOrdered.checked = true;
    } else if (orderRandom) {
        orderRandom.checked = true;
    }

    if (orderRandom) {
        orderRandom.addEventListener('change', (e) => {
            if (e.target.checked) localStorage.setItem('questionOrder', 'random');
        });
    }
    if (orderOrdered) {
        orderOrdered.addEventListener('change', (e) => {
            if (e.target.checked) localStorage.setItem('questionOrder', 'ordered');
        });
    }
    ...
    if (perfectCheckbox) {
        perfectCheckbox.addEventListener('change', (e) => {
            ...
            // 即時保存
            if (typeof saveQuizProgressToServer === 'function') {
                saveQuizProgressToServer().catch(err => console.error('Error saving perfect status:', err));
            }
        });
    }
    ...
    if (durationSelect) {
        durationSelect.addEventListener('change', (e) => {
            if (perfectCheckbox && perfectCheckbox.checked) {
                perfectCheckbox.dispatchEvent(new Event('change'));
            }
        });
    }
});
